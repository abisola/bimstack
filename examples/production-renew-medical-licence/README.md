...
Production-ready front-end for the medical-licence renewal service, generated from `examples/build-renew-medical-licence/prototype-1-phone-first/iteration-3/index.html` via `/bimstack:productionise`. Includes a comprehensive test suite – regression, accessibility, security, and load.
...
```
brief → /bimstack:discover → /bimstack:build → /bimstack:iterate (× N) → /bimstack:productionise → /bimstack:assess for beta gate → private beta → public beta → live
                                                  ↑                  ↑
...
3. Refreshes the threat model with `/bimstack:threat-model`
4. Runs the k6 peak test against staging
5. Schedules the pen test
6. Runs `/bimstack:assess for the beta gate` with the delivery manager – this report is the front-end evidence

## Licence and provenance

Generated by bimstack v0.1. MIT licensed. Built end-to-end from a brief through three iteration rounds and a productionise step using `/bimstack:discover`, `/bimstack:build`, `/bimstack:iterate`, and `/bimstack:productionise`.
